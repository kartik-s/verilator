...
public:
    void emitModule(AstNodeModule* modp) {
        ...
        embed->puts("};\n");
        embed->puts("void *" + prefix + "_new(void* contextp);\n");
        ...
        m_ofp->puts("__index = {\n");
        m_ofp->puts("new = function(context)"
    ...
    void emitShim(AstNodeModule* modp) {
        ...
        // Emit constructor
        m_ofp->puts("void *" + prefix + "_new(void* contextp) {\n");
        m_ofp->puts("return reinterpret_cast<void *>(new " + prefix
                    + "(reinterpret_cast<VerilatedContext*>(contextp)));\n");
        m_ofp->puts("}\n");

        // Emit destructor
        m_ofp->puts("void " + prefix + "_delete(void *p) {\n");
        m_ofp->puts("delete reinterpret_cast<" + prefix + " *>(p);\n");
        m_ofp->puts("}\n");

        // Emit eval
        m_ofp->puts("void " + prefix + "_eval(void *p) {\n");
        m_ofp->puts("return reinterpret_cast<" + prefix + " *>(p)->eval();\n");
        m_ofp->puts("}\n");

        // Emit eval_step
        m_ofp->puts("void " + prefix + "_eval_step(void *p) {\n");
        m_ofp->puts("return reinterpret_cast<" + prefix + " *>(p)->eval_step();\n");
        m_ofp->puts("}\n");

        // Emit eval_end_step
        m_ofp->puts("void " + prefix + "_eval_end_step(void *p) {\n");
        m_ofp->puts("return reinterpret_cast<" + prefix + " *>(p)->eval_end_step();\n");
        m_ofp->puts("}\n");

        // Emit final
        m_ofp->puts("void " + prefix + "_final(void *p) {\n");
        m_ofp->puts("return reinterpret_cast<" + prefix + " *>(p)->final();\n");
        m_ofp->puts("}\n");

        // Emit got_finish
        m_ofp->puts("bool " + prefix + "_got_finish(void *p) {\n");
        m_ofp->puts("return reinterpret_cast<" + prefix + " *>(p)->contextp()->gotFinish();\n");
        m_ofp->puts("}\n");
